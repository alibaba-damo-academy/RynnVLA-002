...
def run_script(rank, all_ranks, data_t, task, resolution): # 添加 task 参数
    ...
    #            f"--splits={all_ranks} "
    #            f"--rank={rank} "
    #            f"--in_filename /mnt/PLNAS/cenjun/processed_lerobot_data_rel_state/convs/{task}.json "
    #            f"--out_dir /mnt/PLNAS/cenjun/processed_lerobot_data_rel_state_256_q0199/tokens/{task} "
    ...
    #            f"--out_dir /mnt/PLNAS/cenjun/all_data/extracted/processed_lerobot_data_rel_state_256_minmax/tokens/{task} "
    #            f"--target_size {resolution}")

    os.system(f"python -u pre_tokenize_action_local_2.py "
            f"--splits={all_ranks} "
            f"--rank={rank} "
            f"--in_filename /mnt/PLNAS/cenjun/all_data/extracted/convs/{task}.json "
            f"--out_dir /mnt/PLNAS/cenjun/all_data/extracted/processed_lerobot_data_rel_state_256_minmax/tokens/{task} "
